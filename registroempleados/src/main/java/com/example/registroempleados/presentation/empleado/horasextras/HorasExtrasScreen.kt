...
fun HorasExtrasScreen(
    ...
    Scaffold(
        ...
        // Aquí validamos con EXPANDED en mayúsculas
        if (windowSizeClass == WindowWidthSizeClass.EXPANDED) {
            Row(
                ...
                Column(
                    ...
                    FormularioInputs(state, viewModel::onEvent)
                }
                Column(modifier = Modifier.weight(1f)) {
                    TarjetaResultado(state)
                }
            }
        } else {
            Column(
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun FormularioInputs(
    state: HorasExtrasUiState,
    onEvent: (HorasExtrasUiEvent) -> Unit
) {
    ExposedDropdownMenuBox(
        expanded = state.isDropdownExpanded,
        onExpandedChange = { onEvent(HorasExtrasUiEvent.OnDropdownExpandedChanged(it)) }
    ) {
        OutlinedTextField(
        ...
        ExposedDropdownMenu(
            expanded = state.isDropdownExpanded,
            onDismissRequest = { onEvent(HorasExtrasUiEvent.OnDropdownExpandedChanged(false)) }
        ) {
            state.empleados.forEach { empleado ->
                DropdownMenuItem(
                    text = { Text("${empleado.nombres} - $${empleado.sueldo}") },
                    onClick = { onEvent(HorasExtrasUiEvent.OnEmpleadoSelected(empleado)) }
                )
            }
        }
    }
    ...
    OutlinedTextField(
        ...
        onValueChange = { onEvent(HorasExtrasUiEvent.OnHorasSemanalesChanged(it)) },
    ...
    OutlinedTextField(
        ...
        onValueChange = { onEvent(HorasExtrasUiEvent.OnHorasNocturnasChanged(it)) },
    ...
    Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
        OutlinedButton(
            onClick = { onEvent(HorasExtrasUiEvent.Limpiar) },
            modifier = Modifier.weight(1f)
        ) {
            Text("Limpiar")
        }
        Button(
            onClick = { onEvent(HorasExtrasUiEvent.Calcular) },
            modifier = Modifier.weight(1f),
            enabled = state.selectedEmpleado != null
        ) {
            Text("Calcular")
...
@Composable
fun TarjetaResultado(state: HorasExtrasUiState) {
